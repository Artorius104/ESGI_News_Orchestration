"""Comparaison de modeles par GridSearchCV avec explicabilite SHAP (S7).

Compare RF / XGBoost / LightGBM via GridSearchCV et persiste le meilleur
dans models/model.joblib. Chaque famille est tracee dans MLflow (runs
imbriques) et la meilleure est enregistree dans le Model Registry.
...
def main() -> None:
    ...
    parser.add_argument("--scoring", type=str, default=SCORING)
    parser.add_argument(
        "--no-mlflow",
        dest="use_mlflow",
        action="store_false",
